...
resource "aws_instance" "app_single" {
  ...
  vpc_security_group_ids = ["${aws_security_group.allow_all_bc.id}"]
...
resource "aws_security_group" "allow_all_bc" {
  name        = "allow_all_bc"
  description = "Allow all inbound traffic (blockchain testing)"
